...
#   v2.2 - 2025-08-17 - CORRECTION détection fichiers vidéo dans --folder mode
...
LOG_FILE="log.transcribe_mp4.v2.2.log"
ACTIONS_LOG=()
...
# Fonction pour détecter les extensions vidéo - VERSION CORRIGÉE
is_video_file() {
    local file="$1"
...
# Fonction de traitement par lot d'un dossier - VERSION CORRIGÉE
process_folder() {
    local folder="$1"
    ...
    # Recherche des fichiers vidéo - MÉTHODE AMÉLIORÉE ET DEBUGGÉE
    local video_files=()
    local total_files=0
    ...
    # DEBUG: Afficher le contenu complet du dossier
    echo "📂 DEBUG - Contenu du dossier '$folder' :"
    if ! ls -la "$folder" 2>/dev/null; then
        echo "❌ ERREUR : Impossible de lire le contenu du répertoire"
        return 1
    fi
    echo ""

    # Parcourir tous les fichiers du dossier avec gestion d'erreurs renforcée
    shopt -s nullglob # Évite les erreurs si aucun fichier ne correspond

    for file in "$folder"/*; do
        # DEBUG: Afficher chaque fichier examiné
        echo "🔍 Examen: $file"

        # Vérifier que c'est un fichier (pas un dossier) et qu'il existe
        if [ ! -f "$file" ]; then
            echo "   ➜ Ignoré (pas un fichier régulier)"
            continue
        fi
        ...
        # Vérifier qu'il y a bien une extension
        if [[ "$filename" != *.* ]]; then
            echo "   ➜ Ignoré (pas d'extension détectée)"
            continue
        fi

        local extension="${filename##*.}"
        local extension_lower="${extension,,}" # conversion en minuscules
        echo "   ➜ Extension détectée: '$extension' -> '$extension_lower'"

        # Vérifier si l'extension est dans notre liste supportée
        local is_video=false
        for supported_ext in "${VIDEO_EXTENSIONS[@]}"; do
            if [ "$extension_lower" = "$supported_ext" ]; then
                is_video=true
                break
            fi
        done
        ...
        if [ "$is_video" = true ]; then
            video_files+=("$file")
            ((total_files++))
            echo "   ✅ ACCEPTÉ: $filename (extension .$extension_lower)"
        else
            echo "   ➜ Ignoré (extension non supportée: .$extension_lower)"
        fi
        echo ""
    done

    shopt -u nullglob # Remettre le comportement par défaut

    echo "🎯 RÉSULTAT DE LA RECHERCHE :"
    echo "   Fichiers vidéo trouvés : $total_files"
    echo ""
    ...
    if [ "$total_files" -eq 0 ]; then
        ...
        echo "1. Extensions recherchées vs extensions présentes :"
        echo "   Extensions supportées : ${VIDEO_EXTENSIONS[*]}"
        echo "   Extensions trouvées dans le dossier :"
        ...
        for file in "$folder"/*; do
            if [ -f "$file" ]; then
                ...
                if [[ "$filename" == *.* ]]; then
                    ...
                    local ext_lower="${ext,,}"
                    # Éviter les doublons
                    if [[ ! " ${found_extensions[*]} " =~ " ${ext_lower} " ]]; then
                        found_extensions+=("$ext_lower")
                    fi
                fi
            fi
        done

        if [ "${#found_extensions[@]}" -gt 0 ]; then
            echo "     .$( printf ".%s " "${found_extensions[@]}" )"
        else
            echo "     (aucune extension détectée)"
        fi
        echo ""

        echo "2. Suggestions de résolution :"
        echo "   • Vérifiez le chemin du répertoire : '$folder'"
        echo "   • Vérifiez que les fichiers ont bien une extension vidéo"
        echo "   • Extensions supportées : mp4, avi, mkv, mov, wmv, flv, webm, m4v, 3gp, ogv, ts, mts, m2ts"
        echo "   • Les extensions sont insensibles à la casse (MP4 = mp4)"
        echo ""

        return 1
    fi

    echo "✅ FICHIERS VIDÉO DÉTECTÉS ($total_files) :"
